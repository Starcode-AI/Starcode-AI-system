...
* **Code & Infrastructure:** Licensed under the [GNU GPLv3](./LICENSE).
* **Model Weights & Updates:** Licensed under the [OpenRAIL-M](./MODEL_LICENSE). *Any locally trained weight updates shared with others must remain under this license.*
...
2. **Install dependencies & launch:**
   *(Please refer to the installation guide in the `/docs` folder for hardware-specific prerequisites.)*

### 🛡️ Security & Support
If you discover any security vulnerabilities, please contact us privately at **support@starcode-bom.com**. For more details, see our [Security Policy](./SECURITY.md).
...
* **Code & Infrastruktur:** Lizenziert unter der [GNU GPLv3](./LICENSE).
* **Modellgewichte & Updates:** Lizenziert unter der [OpenRAIL-M](./MODEL_LICENSE). *Alle lokal generierten Gewichts-Updates, die weitergegeben werden, müssen unter dieser Lizenz bleiben.*
...
2. **Abhängigkeiten installieren & starten:**
   *(Bitte beachten Sie die Installationsanleitung im Ordner `/docs` für hardware-spezifische Voraussetzungen.)*

### 🛡️ Sicherheit & Support
Wenn Sie Sicherheitslücken entdecken, melden Sie diese bitte vertraulich per E-Mail an **support@starcode-bom.com**. Weitere Details finden Sie in unserer [Sicherheitsrichtlinie](./SECURITY.md).
